...
export default async function CategoryPage({
	...
	const { category } = await getCategoryById({ id }); // カテゴリー名を取得

	return (
		<MainAndAside>
			<h2 className="mx-8 text-2xl font-bold mb-2">{`「${category}」カテゴリー（ ${posts.totalCount}件 ）`}</h2>
...
export const generateStaticParams = async () => {
	const categoryList = await getAllCategoryIds();

	const paths = categoryList.map((id) => ({
		id, // タグのIDを使用
	}));
